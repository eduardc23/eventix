  ...
  group('BookingFailureMessageX - Resolución de Mensajes', () {
    group('BookingFailure - Errores de Reserva', () {
      test('Presenta mensaje de error cuando no hay cupos disponibles', () {
        ...
        expect(failure.toBookingMessage, equals(BookingStrings.noSpotsMessage));
      });
    });
    ...
    group('CoreFailure - Delegación de Errores', () {
      test('Delega correctamente el mensaje ante un fallo de servidor', () {
        ...
        expect(failure.toBookingMessage, equals('Error de servidor custom'));
      });

      test('Delega correctamente el mensaje ante un fallo de red', () {
        const failure = NetworkFailure();
        expect(failure.toBookingMessage, equals(AppConstants.networkError));
      });

      test('Delega correctamente el mensaje ante un fallo de tiempo de espera', () {
        const failure = TimeoutFailure();
        expect(failure.toBookingMessage, equals(AppConstants.timeoutError));
      });
    });
    ...
    group('Fallback - Casos No Manejados', () {
      test('Utiliza el mensaje de error inesperado ante cualquier otro tipo de fallo', () {
        ...
        expect(unknownFailure.toBookingMessage, equals(BookingStrings.unexpectedBookingError));
      });
    });
  });
}
